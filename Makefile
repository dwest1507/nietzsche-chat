...
help:
	...
	@echo "  make build-index             - Build FAISS + BM25 indexes from the Nietzsche corpus"
...
# frontend left running against a dead backend just serves "Nietzsche is
# unreachable" with no hint that the backend is the part that died.
dev: frontend-deps
