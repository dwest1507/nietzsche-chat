      ...
      <div className={`flex max-w-[85%] flex-col gap-2 ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        >
          {isUser ? (
            ...
            <div
              className={`text-[14px] leading-relaxed break-words ${streaming ? '[&>p:last-child]:inline' : ''}`}
            >
              <ReactMarkdown
                ...
                components={{
                  ...
                  h4: ({ children }) => (
                    <h4 className="mt-2 mb-1 text-[14px] font-medium text-white first:mt-0">
                      {children}
                    </h4>
                  ),
                  ...
                  th: ({ children }) => (
                    <th className="bg-white/[0.04] px-3 py-1.5 font-medium text-white">
                      {children}
                    </th>
                  ),
                  td: ({ children }) => (
                    <td className="border-t border-white/[0.06] px-3 py-1.5">{children}</td>
                  ),
                }}
              >
                {message.content}
